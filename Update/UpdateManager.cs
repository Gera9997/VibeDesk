...
namespace VibeDesk.Update
{
    ...
    public class UpdateManager
    {
        ...
        {
            _httpClient.DefaultRequestHeaders.Add("User-Agent", "VibeDesk-AutoUpdater");
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            string updateFilePath = Path.Combine(Path.GetDirectoryName(currentExe)!, "VibeDesk_update.exe");
        ...
        private static void ApplyUpdateAndRestart(string newFilePath, string currentExe)
        {
            ...
            // 1. Rename running exe to backup
            File.Move(currentExe, backupPath, overwrite: true);

            // 2. Move new file into place
            File.Move(newFilePath, currentExe, overwrite: true);

            // 3. Launch background cmd command to wait 1 second, delete .bak, and launch updated exe
            string cmdArgs = $"/c timeout /t 1 /nobreak > nul & del /f /q \"{backupPath}\" & start \"\" \"{currentExe}\"";
